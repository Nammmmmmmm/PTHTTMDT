...
const EditProductModal = ({ product, onClose, onUpdate }) => {
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      ...
      console.log('Đang xóa sản phẩm:', editingProduct.name, 'ID:', editingProduct._id || editingProduct.id);
      ...
      const productToDelete = {
        ...
        is_hot: editingProduct.is_hot || false,
        is_feature: editingProduct.is_feature || false,
        is_delete: true  // Đánh dấu xóa
      };

      console.log('Dữ liệu gửi lên API:', productToDelete);

      // Nếu có thumbnail URL và không thay đổi
      if (editingProduct.thumbnail && !imageFile && !editingProduct.thumbnail.startsWith('blob:')) {
        productToDelete.thumbnail_url = editingProduct.thumbnail;
      }
      ...
      if (imageFile) {
        ...
        // Thêm dữ liệu cơ bản vào FormData
        Object.keys(productToDelete).forEach(key => {
          if (key === 'category_id') {
            // Xử lý đặc biệt cho mảng category_id
            productToDelete[key].forEach((catId, index) => {
              formData.append(`category_id[${index}]`, catId);
            });
          } else {
            formData.append(key, productToDelete[key]);
          }
        });
      ...
      console.log('Phản hồi từ API:', updatedProduct);
      
      // Đảm bảo sản phẩm có is_delete = true
      const finalProduct = {
        ...updatedProduct,
        is_delete: true,
      ...
      console.log('Sản phẩm cuối cùng để cập nhật state:', finalProduct);
      ...
      setFormErrors({
        submit: error.response?.data?.message || 'Lỗi khi xóa sản phẩm. Vui lòng thử lại.'
      });
    } finally {
      setLoading(false);
    }
...
const ProductList = () => {
  ...
  const sortedProducts = useMemo(() => {
    console.log('sortedProducts useMemo được gọi, tổng số sản phẩm:', products.length);
    
    // Lọc sản phẩm theo trạng thái và trạng thái xóa
    let filteredProducts = [...products];
    
    console.log('Sản phẩm trước khi lọc is_delete:', filteredProducts.map(p => ({name: p.name, is_delete: p.is_delete})));
    
    // Chỉ hiển thị các sản phẩm không bị xóa hoàn toàn
    filteredProducts = filteredProducts.filter(product => {
      const shouldShow = !product.is_delete;
      if (!shouldShow) {
        console.log('Ẩn sản phẩm đã xóa:', product.name, 'is_delete:', product.is_delete);
      }
      return shouldShow;
    });
    
    console.log('Sản phẩm sau khi lọc is_delete:', filteredProducts.length);
    ...
    console.log('Sản phẩm cuối cùng hiển thị:', filteredProducts.length);
  ...
  const handleUpdateProduct = (updatedProduct) => {
    console.log('handleUpdateProduct được gọi với:', updatedProduct);
    
    setProducts(prevProducts => {
      const newProducts = prevProducts.map(product => {
        if (product.id === updatedProduct.id || product.id === updatedProduct._id || 
            product._id === updatedProduct.id || product._id === updatedProduct._id) {
          
          const updatedItem = {
            ...product, // Giữ lại dữ liệu cũ
            ...updatedProduct, // Ghi đè với dữ liệu mới
            id: updatedProduct.id || updatedProduct._id || product.id,
            _id: updatedProduct._id || updatedProduct.id || product._id
          };
          
          console.log('Cập nhật sản phẩm:', product.name, 'is_delete:', updatedItem.is_delete);
          return updatedItem;
        }
        return product;
      });
      
      console.log('Products sau khi cập nhật:', newProducts.filter(p => p.id === updatedProduct.id || p.id === updatedProduct._id));
      return newProducts;
    });
  };
