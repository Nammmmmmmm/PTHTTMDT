  ...
  const fetchConversations = async () => {
    try {
      ...
      if (!currentUser || !currentUser.id) {
        console.log('[fetchConversations] User not logged in');
      ...
      const token = AuthService.getToken();
      if (!token) {
        console.log('[fetchConversations] No token found');
        setError('Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại');
        setLoading(false);
        return;
      }

      console.log('[fetchConversations] Starting to fetch conversations for user:', currentUser.id);
      setLoading(true);
