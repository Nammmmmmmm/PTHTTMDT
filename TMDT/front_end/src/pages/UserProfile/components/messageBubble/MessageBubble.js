  ...
  const fetchConversations = async () => {
    try {
      ...
      if (!currentUser || !currentUser.id) {
        console.log('[MessageBubble fetchConversations] User not logged in');
      ...
      if (!token) {
        console.log('[MessageBubble fetchConversations] No token found');
        setError('Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại');
        setLoading(false);
        return;
      }

      console.log('[MessageBubble fetchConversations] Starting to fetch conversations for user:', currentUser.id);
      setLoading(true);
