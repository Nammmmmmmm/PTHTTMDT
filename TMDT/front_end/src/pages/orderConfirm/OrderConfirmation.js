...
import { CheckCircle, Package, Truck, Home, AlertCircle, XCircle, ChevronLeft, ChevronRight, Store, RefreshCw } from 'lucide-react';
...
const OrderConfirmation = () => {
  ...
  // Xử lý clear cart khi thanh toán online thành công
  useEffect(() => {
    const clearCartAfterSuccessfulPayment = async () => {
      try {
        // Kiểm tra xem có phải là thanh toán online thành công không
        const transactionCode = localStorage.getItem('paymentTransactionCode');
        const currentOrderId = localStorage.getItem('currentOrderId');
        
        if (transactionCode && currentOrderId && orders.length > 0) {
          // Kiểm tra xem có đơn hàng nào là online payment và đã thanh toán thành công
          const hasSuccessfulOnlinePayment = orders.some(orderData => {
            const order = orderData.order;
            return isOnlinePayment(order) && 
                   (order.status_id === 'paid' || order.order_status === 'processing' || order.order_status === 'paid');
          });
          
          if (hasSuccessfulOnlinePayment) {
            console.log('Clearing cart data after successful online payment');
            ...
            // Clear actual cart if we can identify it
            // Try to get cart ID from any order detail that might have it
            let cartIdToClear = null;
            for (const orderData of orders) {
              const orderDetails = orderData.orderDetails || [];
              for (const detail of orderDetails) {
                if (detail.cart_id) {
                  cartIdToClear = detail.cart_id;
                  break;
                }
              }
              if (cartIdToClear) break;
            }
            
            // Clear the cart
            if (cartIdToClear) {
              try {
                await ApiService.delete(`/cart/clear/${cartIdToClear}`);
        ...
        console.error('Error in clearCartAfterSuccessfulPayment:', error);
      }
    };
    
    // Chỉ chạy khi orders đã được load
  ...
  const checkPaymentStatus = async (orderId, transactionCode) => {
    try {
      ...
      if (response && response.success) {
        ...
        // Nếu vẫn pending sau khi check, thử manual update sau 30 giây
        if (response.data.payment.status === "PENDING") {
          console.log("Payment still pending, will try manual update in 30 seconds");
          setTimeout(async () => {
            try {
              console.log("Attempting manual update from PayOS...");
              const manualUpdateResponse = await ApiService.post(`/payos/manual-update/${transactionCode}`, {}, true);
              
              if (manualUpdateResponse && manualUpdateResponse.success) {
                console.log("Manual update successful:", manualUpdateResponse);
                
                // Nếu manual update thành công và status đã thay đổi, reload trang
                if (manualUpdateResponse.data.payment.status === "PAID") {
                  setTimeout(() => {
                    window.location.reload();
                  }, 1000);
                }
  ...
  const manualRefreshPaymentStatus = async (transactionCode) => {
    try {
      setCheckingPayment(true);
      console.log("Manual refresh payment status:", transactionCode);
      
      const response = await ApiService.post(`/payos/manual-update/${transactionCode}`, {}, true);
      
      if (response && response.success) {
        console.log("Manual refresh successful:", response);
        setPaymentStatus(response.data.payment);
        
        // Reload trang nếu trạng thái đã thay đổi thành PAID
        ...
        alert(response.message || "Đã cập nhật trạng thái thanh toán");
      } else {
        alert("Không thể cập nhật trạng thái thanh toán");
      }
    } catch (error) {
      console.error("Manual refresh error:", error);
      alert("Lỗi khi cập nhật trạng thái: " + (error.message || "Không xác định"));
    } finally {
      setCheckingPayment(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto p-6 mt-10">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6 text-center">
        ...
        {isOnlinePayment(order) && order.status_id === 'pending' && (
          <div className="text-yellow-600 mt-2 bg-yellow-50 p-3 rounded-lg">
            <div className="flex items-center mb-2">
              <AlertCircle size={16} className="mr-1" />
              <span>Lưu ý: Đơn hàng của bạn đang chờ thanh toán. Vui lòng hoàn tất thanh toán để đơn hàng được xử lý.</span>
            </div>
            ...
            {order.order_payment_id && (
              <div className="flex items-center justify-center mt-3">
                <button
                  onClick={() => manualRefreshPaymentStatus(order.order_payment_id)}
                  disabled={checkingPayment}
                  className="flex items-center bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white px-4 py-2 rounded-lg text-sm transition-colors"
                >
                  <RefreshCw size={16} className={`mr-2 ${checkingPayment ? 'animate-spin' : ''}`} />
                  {checkingPayment ? 'Đang kiểm tra...' : 'Kiểm tra lại trạng thái thanh toán'}
                </button>
              </div>
            )}
          </div>
