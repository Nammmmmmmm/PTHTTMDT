...
class ApiService {
  ...
  getHeaders(secure) {
    ...
    if (secure) {
      ...
      if (token) {
        headers['x-access-token'] = token;
        console.log(`[ApiService] Token found and added to headers: ${token.substring(0, 20)}...`);
      } else {
        console.log('[ApiService] No token found - request will fail if auth is required');
      }
    }
