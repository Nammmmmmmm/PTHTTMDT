...
passport.use(new GoogleStrategy({
    ...
    try {
        ...
        if (!user) {
            ...
            if (!role) {
                console.error('GoogleStrategy - MEMBER role not found');
                return done(new Error('MEMBER role not found'), null);
            }
            ...
            console.log('GoogleStrategy - New user created:', user.email);
        } else {
            console.log('GoogleStrategy - Existing user found:', user.email);
        }
...
function googleAuthCallback(req, res, next) {
    passport.authenticate('google', { session: false }, async (err, user) => {
        // Debug logging
        console.log('Google auth callback - FRONTEND_URL:', process.env.FRONTEND_URL);
        console.log('Google auth callback - Error:', err);
        console.log('Google auth callback - User:', user ? 'User found' : 'No user');
        ...
        try {
            ...
            console.log('Redirecting to login with googleAuth data');
            res.redirect(`${frontendURL}/login?googleAuth=${userDataEncoded}`);
        } catch (error) {
