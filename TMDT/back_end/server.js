...
// Middleware timeout toàn cục cho PayOS routes
app.use('/api/payos', (req, res, next) => {
  // Đặt timeout cho PayOS requests để tránh server bị đứng hình
  const timeout = 15000; // 15 giây
  
  req.setTimeout(timeout, () => {
    console.log(`PayOS request timeout: ${req.method} ${req.path}`);
    if (!res.headersSent) {
      res.status(408).json({ 
        success: false, 
        message: 'Request timeout',
        error: 'PayOS request exceeded timeout limit'
      });
    }
  });
  
  res.setTimeout(timeout, () => {
    console.log(`PayOS response timeout: ${req.method} ${req.path}`);
  });
...
app.use((err, req, res, next) => {
  ...
  // Đảm bảo không response nếu headers đã được gửi
  if (res.headersSent) {
    return next(err);
  }
...
process.on('uncaughtException', (error) => {
  ...
  // Không tắt server ngay lập tức, chỉ log lỗi
});
